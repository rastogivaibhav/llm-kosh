...
New cartridges get a `cartridge_id` in `CARTRIDGE.json`. Pre-v0.6 cartridges (no id) get a
...
- `cartridge.py` — rewrote `pack_context`; added `validate_pack`, `explain_pack`,
  `cartridge_meta`, profile/budget helpers; `cartridge_id` in init (version → 0.6.0).
