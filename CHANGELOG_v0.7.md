...
- `policy [--init]` — show the effective export policy, or write a default
  `CARTRIDGE_POLICY.json`. Policy keys: `default_visibility`, `blocked_terms`,
...
- `cartridge.py` — added the policy/classify/partition/quarantine/safe-pack module and the
...
optional fields). Cartridges with no `CARTRIDGE_POLICY.json` use safe built-in defaults, so
