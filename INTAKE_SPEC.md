...
The LlmKosh Intake Control Plane is a unified subsystem that tracks every incoming item (MEMORY_RECEIPTs, text files, daemon intakes, import exports) before they are applied to the core AI memory cartridge.
...
1. **Scan**: `intake scan` discovers new files in `receipts/`, `inbox/`, `attachments/imports/`, and `source/receipts/`. It creates JSON `llm-kosh.intake.v1` records in `intake/pending/`.
...
## Policy
`LLM_KOSH_POLICY.json` supports an `intake` configuration block determining auto-apply behavior:
```json
"intake": {
