...
```bash
python3 koush_cli.py --root $R init --owner "Your Name"
```

This creates the folder tree, `KOUSH.json` (with a `koush_id`), `BOOT.md`, and an
empty index.
...
```bash
python3 koush_cli.py --root $R inbox "Maybe use pgvector for ranking" --project SelectiveOS
python3 koush_cli.py --root $R inbox                      # list pending
python3 koush_cli.py --root $R promote --id <note-id> --to decision
```
...
```bash
python3 koush_cli.py --root $R query "teacher approval"
python3 koush_cli.py --root $R query "ranking" --kind decision --project SelectiveOS
```
...
python3 koush_cli.py --root $R query "lesson sign-off" --semantic
```

## 5. Pack for an LLM

```bash
python3 koush_cli.py --root $R pack "SelectiveOS teacher lessons and registration" \
    --for chatgpt --out selectiveos.zip --include-private
python3 koush_cli.py --root $R explain-pack selectiveos.zip
python3 koush_cli.py --root $R validate-pack selectiveos.zip
```
...
python3 koush_cli.py --root $R resolve         # finish any corrections it couldn't auto-match
```
...
```bash
python3 koush_cli.py --root $R today           # glance at gaps / corrections / packs
python3 koush_cli.py --root $R audit
python3 koush_cli.py --root $R heal --safe
...
```bash
python3 koush_cli.py --root $R export-backup --out cartridge-backup.zip
```
