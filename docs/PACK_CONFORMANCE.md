...
The `.koushpack.zip` is a portable AI context artifact designed to be universally readable by LLMs. To allow external tools, agents, or SaaS platforms to parse and generate these packs reliably, LlmKosh enforces a **Level 0 to Level 3** conformance scale.
...
## How Another Tool Can Support LlmKosh Packs

Third party integrations can interact with LlmKosh by adopting this standard:

1. **Ingest a LlmKosh Pack:**
...
2. **Generate a LlmKosh Pack:**
   If your system outputs context, generate a ZIP adhering to Level 1. Ensure you include `01_BOOT.md` and a receipt template, so if the pack is uploaded manually to an LLM, the output can seamlessly round-trip back to LlmKosh.

## Testing Conformance
Run `llm-kosh conformance generate-sample` to build reference packs locally.
Run `llm-kosh conformance pack <pack.zip>` to test an external pack for validity against the specs.
