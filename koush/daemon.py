...
from koush.engine.commands import heal_safe as do_heal_safe, audit, memory_map
...
from koush.core.utils import read_json, write_json, now_iso
...
def job_process_safe_receipts(root: Path):
    from koush.engine.receipt_trust import review_receipt, trust_receipt, ensure_review_dir
...
def job_regenerate_workbench(root: Path):
    from koush.engine.workbench import build_workbench
...
def daemon_start(root: Path, mode: str):
    ...
    print(f"Starting Koush Daemon (Mode: {mode})")
    
    observer = None
    if mode in ["watchdog", "auto"]:
        if not HAS_WATCHDOG:
            print("Watchdog library missing. Please install it using 'pip install koush[watch]' or run in polling mode.")
            if mode == "watchdog":
                return
        else:
