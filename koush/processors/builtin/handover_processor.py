...
class Processor(ProcessorBase):
    name = "handover_processor"
    description = "Parses agent handover documents into project context"

    def inspect(self, intake_record_or_file: Dict[str, Any], file_path: Path) -> bool:
        if file_path.suffix == ".md" and "handover" in file_path.name.lower():
            return True
        return False
    ...
    def generate_proposal(self, intake_record_or_file: Dict[str, Any], file_path: Path) -> ProposalBatch:
        ...
        rec = ProposalRecord(
            ...
            action="create",
            proposed_kind="note",
            title=f"Handover: {file_path.stem}",
            body=content,
            project="handovers"
        )
