...
class Processor(ProcessorBase):
    name = "safety_processor"
    description = "Detects safety issues or security policies in intake items"

    def inspect(self, intake_record_or_file: Dict[str, Any], file_path: Path) -> bool:
        if file_path.suffix == ".md" and "safety" in file_path.name.lower():
            return True
        return False
    ...
    def generate_proposal(self, intake_record_or_file: Dict[str, Any], file_path: Path) -> ProposalBatch:
        ...
        rec = ProposalRecord(
            ...
            title=f"Safety Policy: {file_path.stem}",
            body=content,
            project="security"
        )
