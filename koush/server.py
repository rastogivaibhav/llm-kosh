...
from koush.engine.commands import heal_safe, audit
...
class CartridgeAPIHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if self.path == '/query':
            ...
            try:
                from koush.engine.commands import status
                f = io.StringIO()
                with redirect_stdout(f):
                    status(root)
                output = f.getvalue()
            ...
            try:
                from koush.engine.commands import classify
...
def start_server(root_path: Path, host: str, port: int):
    ...
    print(f"Koush API Daemon running at http://{host}:{port}")
    try:
