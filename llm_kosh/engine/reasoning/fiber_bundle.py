...
def build_fiber_bundle(
    ...
    # Multi-anchor hyperedge materialization.
    # A hyperedge A ∧ B -> C cannot be discovered by a single DFS branch that
    # starts from A with visited={A}; B is a sibling anchor, not a descendant.
    # If all hyperedge sources are already active in the query candidate/anchor
    # set, materialize joint-causality synthetic paths into the target fiber.
    active_ids = set(anchor_ids) | target_ids
    for he in getattr(dag, "hyperedges", []):
        if he.target_id not in target_ids:
            continue
        if not he.source_ids.issubset(active_ids):
            continue
        # Use public expansion method to preserve provenance and avoid leaking
        # hyperedge internals into this layer.
        target_fact = dag.get_fact(he.target_id)
        if target_fact is None:
            continue
        if he.target_id not in fibers:
            fibers[he.target_id] = Fiber(fact=target_fact, paths=[], degeneracy=0, max_confidence=0.0)
        for source_id in sorted(he.source_ids):
            for edge in dag.get_hyperedge_expansions(source_id, active_ids - {he.target_id}, query_time):
                if edge.target_id == he.target_id:
                    fibers[he.target_id].paths.append(CausalPath(
                        edges=[edge],
    ...
    for fiber in fibers.values():
        fiber.paths = _dedupe_paths(fiber.paths)
...
def _dedupe_paths(paths: List[CausalPath]) -> List[CausalPath]:
    """Deduplicate paths by edge identity while keeping the strongest score."""
    best: Dict[Tuple[str, ...], CausalPath] = {}
    for path in paths:
        key = tuple(edge.id for edge in path.edges)
        current = best.get(key)
        if current is None or path.confidence_product > current.confidence_product:
            best[key] = path
...
def _enumerate_paths(
    ...
    while stack:
        ...
        outgoing = list(dag.get_outgoing_edges(current_id, query_time))
        outgoing.extend(dag.get_hyperedge_expansions(current_id, visited, query_time))
