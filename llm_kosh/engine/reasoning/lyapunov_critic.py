...
class StabilityResult:
    score: float
    status: str            # "stable" | "marginal" | "unstable" | "no_evidence"
...
class LyapunovCritic:
    ...
    V = w1·temporal_consistency + w2·path_diversity + w3·degeneracy - w4·contradiction_score - w5·pattern_lock_score
    ...
    def evaluate(self, bundle: FiberBundle) -> StabilityResult:
        ...
        pattern_lock_score = self._pattern_lock_score(bundle)
        ...
        return StabilityResult(
            ...
            dimensions={
                ...
                "pattern_lock_score": round(pattern_lock_score, 4),
            },
            implicated_facts=[],
            abstain=False,
            reason=None,
    ...
    def _pattern_lock_score(self, bundle: FiberBundle) -> float:
        """High when one path dominates and alternatives are absent."""
        path_counts = [len(fiber.paths) for fiber in bundle.fibers.values()]
        if not path_counts:
            return 1.0
        single_path_fibers = sum(1 for c in path_counts if c <= 1)
        single_ratio = single_path_fibers / len(path_counts)
        total_paths = sum(path_counts)
        if total_paths <= 1 and len(bundle.fibers) <= 1:
            return 1.0
        return min(1.0, single_ratio)
